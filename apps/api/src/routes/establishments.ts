...
export const establishmentsRouter = new Hono()
  ...
  .post('/', zValidator('json', createEstablishmentSchema), async (c) => {
  ...
  .put('/:id', zValidator('json', updateEstablishmentSchema), async (c) => {
    const id = c.req.param('id');
    const data = c.req.valid('json');
    try {
